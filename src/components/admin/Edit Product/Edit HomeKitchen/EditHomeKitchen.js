...
const EditHomeKitchen = () => {
  const [come, setCome] = useState([]);

  const getProduct = async () => {
    const res = await fetch("/gethomekitchens", {
      method: "GET",
      headers: {
          ...
          return (
            ...
            >
              ...
              <div className="productText">
                <div className="productTitle">{e.shortTitle}</div>
              ...
              >
                <NavLink to={`/edithomekitchendata/${e._id}`}>
                  <button
                    style={{
                      cursor: "pointer",
                      height: "40px",
...
export default EditHomeKitchen;
